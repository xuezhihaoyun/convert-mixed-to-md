...
def looks_like_archive_metadata_dump(text: str) -> bool:
    lowered = text.lower()
    markers = [
...
def is_suspicious_pdf_text(text: str, page_count: int) -> bool:
    ...
    avg_chars_per_page = (char_count / page_count) if page_count > 0 else char_count

    # Large PDFs with extremely low extracted text density are usually scanned/image-only.
    low_density_large_pdf = page_count >= 20 and avg_chars_per_page < 80

    # Some archive-embedded PDFs expose only metadata text, not body content.
    metadata_only = looks_like_archive_metadata_dump(stripped) and char_count < 8000

    return low_density_large_pdf or metadata_only
...
def convert_pdf_to_md(source_path: Path, output_path: Path) -> None:
    ...
    if text.strip():
        ...
        if is_suspicious_pdf_text(text, effective_page_count):
            print(
                f"[INFO] PDF 提取到的文本疑似元信息/低密度内容，转入 OCR 流程: {source_path}",
                file=sys.stderr,
            )
            text = ""
        else:
